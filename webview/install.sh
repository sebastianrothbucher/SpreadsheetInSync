...
node -e 'var fs=require("fs"); fs.readFile("webview.htm", "utf-8", function(err, data){console.log(JSON.stringify({"html": data}));});' | curl -X PUT $1/webview -d @-
node -e 'var fs=require("fs"); fs.readFile("webview_controller.js", "utf-8", function(err, data){console.log(JSON.stringify({"html": data}));});' | curl -X PUT $1/webview_controller -d @-
